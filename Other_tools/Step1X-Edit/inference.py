...
class ImageGenerator:
    ...
    def generate_image(
        ...
        if init_image is not None:
            ...
            if self.offload:
                self.ae = self.ae.to(self.device)
            init_imagge = self.ae.encode(init_image.to() * 2 - 1)
            if self.offload:
                self.ae = self.ae.cpu()
                cudagc()
...
class ImageEditor:
    def __init__(self, model_path, size_level=1024, seed=42, num_steps=30,
                 cfg_guidance=7.5, offload=True, quantized=True, lora=None):
        self.model = ImageGenerator(
            ae_path=os.path.join(model_path, 'vae.safetensors'),
            dit_path=os.path.join(model_path, "step1x-edit-i1258-FP8.safetensors"),
    ...
    def edit_image(self, image_path, prompt):
        prompt = "(photorealistic:1.4), (ultra-detailed:1.3), 8K, RAW photo, high resolution, professional lighting, sharp focus, skin pores, intricate details, realistic shadows"+prompt
        image = self.model.generate_image(
            prompt,
            negative_prompt="blurry, cartoon, 3d, (disfigured:1.2), (bad art:1.2), (extra limbs:1.3), watermark, lowres, unnatural lighting",
